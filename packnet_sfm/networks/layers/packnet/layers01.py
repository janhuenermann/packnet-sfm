...
class Conv2D(nn.Module):
    ...
    stride : int
        Stride
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride, activation=True, bias=True):
        super().__init__()
...
class ResidualConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, dropout=None):
        ...
        self.conv2 = Conv2D(out_channels, out_channels, 3, 1, activation=False, bias=False)
...
class PackLayerConv3d(nn.Module):
    ...
    into [B,(r^2)C,H/r,W/r] and then convolves it to produce [B,C,H/r,W/r].
    """
    def __init__(self, in_channels, kernel_size, groups, r=2, d=4):
        """
    ...
    def forward(self, x):
        ...
        x = self.pack(x)
        b, d, h, w = x.shape
        g = self.conv3d.groups
        x = x.view(b, g, d // g, h, w)
        x = self.conv3d(x)
        b, c, d, h, w = x.shape
        x = x.view(b, c * d, h, w)
        x = self.conv(x)
